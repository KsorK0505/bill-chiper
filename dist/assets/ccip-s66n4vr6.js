...
class R extends l {
  constructor({ result: r, url: a }) {
    super(
      ...
      {
        metaMessages: [`Gateway URL: ${m(a)}`, `Response: ${y(r)}`],
...
async function C(c, { blockNumber: r, blockTag: a, data: o, to: i }) {
  const { args: d } = k({ data: o, abi: [$] }),
    [t, n, f, u, s] = d,
    { ccipRead: e } = c,
    g =
      e && typeof (e == null ? void 0 : e.request) == "function"
        ? e.request
        : T;
  try {
    if (!b(i, t)) throw new S({ sender: t, to: i });
    const p = await g({ data: f, sender: t, urls: n }),
      { data: w } = await O(c, {
        blockNumber: r,
        blockTag: a,
        data: E([u, L([{ type: "bytes" }, { type: "bytes" }], [p, s])]),
        to: i,
      });
    return w;
  } catch (p) {
...
async function T({ data: c, sender: r, urls: a }) {
  ...
  for (let d = 0; d < a.length; d++) {
    const t = a[d],
      n = t.includes("{data}") ? "GET" : "POST",
      f = n === "POST" ? { data: c, sender: r } : void 0,
      u = n === "POST" ? { "Content-Type": "application/json" } : {};
    try {
      ...
      if (!x(e)) {
        o = new R({ result: e, url: t });
        continue;
      }
      return e;
    } catch (s) {
      o = new h({ body: f, details: s.message, url: t });
    }
  }
  throw o;
}
