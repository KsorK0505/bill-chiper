import { r as T } from "./index-B39l1tw3.js";
import { h as y, j as k, k as I } from "./index-RmsPT7oH.js";
const q = () => "9.1.0",
  ...
  R = () => (typeof window < "u" ? C(10) : new Date().getTime().toString(36));
class g {}
g.makeRequest = (t, e) => ({
  id: R(),
  method: t,
...
g.makeResponse = (t, e, s) => ({ id: t, success: !0, version: s, data: e });
...
var r;
(function (t) {
  (t.sendTransactions = "sendTransactions"),
    (t.rpcCall = "rpcCall"),
    (t.getChainInfo = "getChainInfo"),
    (t.getSafeInfo = "getSafeInfo"),
    (t.getTxBySafeTxHash = "getTxBySafeTxHash"),
    (t.getSafeBalances = "getSafeBalances"),
    (t.signMessage = "signMessage"),
    (t.signTypedMessage = "signTypedMessage"),
    (t.getEnvironmentInfo = "getEnvironmentInfo"),
    (t.getOffChainSignature = "getOffChainSignature"),
    (t.requestAddressBook = "requestAddressBook"),
    (t.wallet_getPermissions = "wallet_getPermissions"),
    (t.wallet_requestPermissions = "wallet_requestPermissions");
})(r || (r = {}));
var m;
(function (t) {
  t.requestAddressBook = "requestAddressBook";
...
class H {
  constructor(e = null, s = !1) {
    (this.allowedOrigins = null),
      ...
      (this.isValidMessage = ({ origin: a, data: n, source: i }) => {
        const o = !n,
          l = !this.isServer && i === window.parent,
          u = typeof n.version < "u" && parseInt(n.version.split(".")[0]),
          B = typeof u == "number" && u >= 1;
        let p = !0;
        return (
          Array.isArray(this.allowedOrigins) &&
            (p = this.allowedOrigins.find((P) => P.test(a)) !== void 0),
          !o && l && B && p
        );
      }),
      (this.logIncomingMessage = (a) => {
        console.info(
          `Safe Apps SDK v1: A message was received from origin ${a.origin}. `,
          a.data,
        );
      }),
      (this.onParentMessage = (a) => {
        this.isValidMessage(a) &&
          (this.debugMode && this.logIncomingMessage(a),
          this.handleIncomingMessage(a.data));
      }),
      (this.handleIncomingMessage = (a) => {
        const { id: n } = a,
          i = this.callbacks.get(n);
...
class E {
  ...
  async getBySafeTxHash(e) {
    ...
    return (
      await this.communicator.send(r.getTxBySafeTxHash, { safeTxHash: e })
    ).data;
  }
  async signMessage(e) {
    const s = { message: e };
  ...
  async signTypedMessage(e) {
    if (!b(e)) throw new Error("Invalid typed data");
    return (await this.communicator.send(r.signTypedMessage, { typedData: e }))
      .data;
  }
  async send({ txs: e, params: s }) {
    if (!e || !e.length) throw new Error("No transactions were passed");
    const a = { txs: e, params: s };
...
class A {
  constructor(e) {
    (this.communicator = e),
      ...
      (this.getPastLogs = this.buildRequest({ call: c.eth_getLogs })),
      ...
      (this.getGasPrice = this.buildRequest({ call: c.eth_gasPrice })),
      (this.getEstimateGas = (s) =>
        this.buildRequest({ call: c.eth_estimateGas })([s])),
      (this.setSafeSettings = this.buildRequest({ call: c.safe_setSettings }));
  }
  buildRequest(e) {
...
class S {
  ...
  async requestPermissions(e) {
    ...
    try {
      return (await this.communicator.send(r.wallet_requestPermissions, e))
        .data;
    } catch {
  ...
  isPermissionRequestValid(e) {
    return e.every((s) =>
      typeof s == "object"
        ? Object.keys(s).every((a) => !!Object.values(m).includes(a))
        : !1,
    );
  }
}
const w = (t, e) => e.some((s) => s.parentCapability === t),
  V = () => (t, e, s) => {
    const a = s.value;
    return (
      (s.value = async function () {
        ...
        if (
          (w(e, i) || (i = await n.requestPermissions([{ [e]: {} }])), !w(e, i))
        )
          throw new d("Permissions rejected", f);
        return a.apply(this);
      }),
...
var M = function (t, e, s, a) {
  var n = arguments.length,
    i =
      n < 3 ? e : a === null ? (a = Object.getOwnPropertyDescriptor(e, s)) : a,
    o;
  if (typeof Reflect == "object" && typeof Reflect.decorate == "function")
    i = Reflect.decorate(t, e, s, a);
  else
    for (var l = t.length - 1; l >= 0; l--)
      (o = t[l]) && (i = (n < 3 ? o(i) : n > 3 ? o(e, s, i) : o(e, s)) || i);
...
class _ {
  ...
  async experimental_getBalances({ currency: e = "usd" } = {}) {
    return (await this.communicator.send(r.getSafeBalances, { currency: e }))
      .data;
  }
  async check1271Signature(e, s = "0x") {
    const a = await this.getInfo(),
      n = y({
        abi: [
          {
            constant: !1,
      ...
      i = {
        call: c.eth_call,
        params: [{ to: a.safeAddress, data: n }, "latest"],
      };
    try {
      return (
        (await this.communicator.send(r.rpcCall, i)).data
          .slice(0, 10)
          .toLowerCase() === x
      );
    } catch {
  ...
  calculateTypedMessageHash(e) {
    const s =
      typeof e.domain.chainId == "object"
        ? e.domain.chainId.toNumber()
        : Number(e.domain.chainId);
    let a = e.primaryType;
    if (!a) {
      const n = Object.values(e.types),
        i = Object.keys(e.types).filter((o) =>
          n.every((l) =>
            l.every(({ type: u }) => u.replace("[", "").replace("]", "") !== o),
          ),
        );
  ...
  async isMessageHashSigned(e, s = "0x") {
    ...
    for (const n of a) if (await n(e, s)) return !0;
    return !1;
  }
...
M([V()], _.prototype, "requestAddressBook", null);
class D {
  constructor(e = {}) {
    const { allowedDomains: s = null, debug: a = !1 } = e;
    (this.communicator = new H(s, a)),
      (this.eth = new A(this.communicator)),
      (this.txs = new E(this.communicator)),
      (this.safe = new _(this.communicator)),
      (this.wallet = new S(this.communicator));
  }
}
export {
